...
```
Distribution for this particular sample is slightly right-skweed.  A typical size (occuring the most) is about 1600.


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
No I would not expect another random sample leading to the same results.  We have no indication of what type of distribution is the population.  And although sample size 60 is fairly large, an other sample could have quite a bit of variation.  Only when we get a ramdom sample distribution would the distribution normalized.
...
---vb---  
The n observation in the sample must be independent.  One way to insure this is to conduct 
random sample with sample size <10% of population.  
--------  
...
---vb---
We are 95% confident that a average living area of house would fall within the  
confidence interval.  
--------  
...
---vb---
My interval (1539.573, 1806.461) did not capture the population mean of 1499.69.
--------  
...
---vb---
I would expect that 95% of the interval would capture the true mean.
Since we are building the interval with a 5% rate of error, meaning
the probability that the true mean of the population is not in the interval
is 5%.  Hence we have 95% chance to have it in the interval.
--------
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```
There are 3 interval out of 50 that does not have the true mean of the population.  Hence we have 6% of failure.
We were expecting 5% of failure.  The results are close but not the same.  Possily due to skweeness in the population.


-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
We will pick a 99% interval.  The critical value = 2.58
...
```
In this instance, we have one interval for which the true mean of the population falls outside.  This represent of 2% rate of failure.  Again, we are close but expected that to be 1%.

```{r, eval = TRUE}
hist(population, breaks = 25)
```
Looking at the histogram of the population, we can see that the distribution of the population appears to be skeewed to the right.  This may account for our interval to be failing to capture the mean with the measure of confidence selected.
