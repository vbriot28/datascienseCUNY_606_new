...
The distribution of this sample is not as skweed as the distribution of the population  
(area).  
--------  
...
# samp2 evaluation = 1509.86, very close to true mean of population of 1499.69

samp3 <- sample(area, 100)
mean(samp3)

# samp3 evaluation = 1580.26

samp4 <- sample(area, 1000)
mean(samp4)

# samp4 evaluation = 1501.768
```

---vb---   
The larger sample provided a better approximation of the population mean.  
--------  
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?
---vb---
There are 5,000 elements in the 'sample_mean50'.  The sampling distribution
is symetrical, centered around 1500.  No, I would not expect the distribution 
to change overmuch.  (I am reading this as 50,000 samples of size 50)  
--------  
...
---vb---
As the sample size is larger, the center is closer to the actual mean of the population and the spread is narrower.  
--------  
...
```{r, eval = TRUE}
# initialize vector of 5000 element
sample_means50 <- rep(NA, 5000)
for (i in 1:5000){
  samp <- sample(price, 50)       # create random sample of size 50 from price population
  sample_means50[i] <- mean(samp) #store the mean of the ith sample into ith position of vector
}
...
The sampling distribution is fairly symetrical although slightly right-skewed, centered around value of 180000.  
The mean of this distribution is $`r mean(sample_means50)`$.
...
```
The distribution for sample distribution with sample size 150 is slightly more symetrical with a center around 180200.  
I would approximate the mean to 18200 for this distribution.  The spread of 150 sample size distribution is much less spread thatn the size 50 sampling distribution.
...
The sampling distribution for sample size 150 has a much smaller spread.  I would rather have a distribution with a smaller spread.
